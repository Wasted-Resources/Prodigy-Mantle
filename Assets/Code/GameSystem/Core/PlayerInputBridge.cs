...
public class PlayerInputBridge : MonoBehaviour
{
    ...
    [Header("Events")]
    [SerializeField] private GameEvent _primaryEvent;
    [SerializeField] private GameEvent _secondaryEvent;
    [SerializeField] private GameEvent _specialEvent;
    ...
    /// <summary>
    /// Subscription service would be nice, but i failed
    /// </summary>
    private void Awake()
    {
        _controls = new();
        // MOVEMENT
        _controls.Player.Move.performed += ctx => _moveInput.Value = ctx.ReadValue<Vector2>();
        _controls.Player.Move.canceled += ctx => _moveInput.Value = Vector2.zero;

        // JUMP
        _controls.Player.Jump.started += ctx =>  _isJumping.Value = true;
        _controls.Player.Jump.canceled += ctx => _isJumping.Value = false;

        // DASH
        _controls.Player.Dash.started += ctx => _isDashing.Value = true;
        _controls.Player.Dash.canceled += ctx => _isDashing.Value = false;

        // ONE-SHOT SIGNALS
        _controls.Player.Primary.started += _ => _primaryEvent?.Raise();
        _controls.Player.Secondary.started += _ => _secondaryEvent?.Raise();
        _controls.Player.Interact.started += _ => _interactEvent?.Raise();
        _controls.Player.Reload.started += _ => _reloadEvent?.Raise();
        _controls.Player.Special.started += _ => _specialEvent?.Raise();
